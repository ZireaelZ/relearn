...
def bert_tokenizer_tensor(batch):
    ...
    for tokenized_text in tokenized_texts:
        ...
        batchout.append(sentence+[0]*(maxlen-len(sentence)))
        tokentypeout.append(tokentypeid+[0]*(maxlen-len(tokentypeid)))
        maskout.append(mask+[0]*(maxlen-len(mask)))
    batchout=torch.tensor(batchout)
    maskout=torch.tensor(maskout)
    tokentypeout=torch.tensor(tokentypeout)
    return  batchout,maskout,tokentypeout
...
# FOR bert
def cal_loss_forBert(logits, targets, mask):
    """计算损失
        参数:
            logits: [B, L, out_size]
            targets: [B, L]
...
def my_cal_loss(logits, targets, tag2id):
    #logis[B,L,tags]
    #targets[B,L]
    #tag2id[]
    PAD = tag2id.get('<pad>')
